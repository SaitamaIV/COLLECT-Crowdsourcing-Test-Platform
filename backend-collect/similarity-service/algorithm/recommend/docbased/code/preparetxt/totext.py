...
def save_file(dirname,save_file,picklefile):
    f_train = io.open(save_file, 'w', encoding='utf-8')
    ...
    with open(picklefile, "wb") as pkf:
        pickle.dump(dictname_list,pkf)
    f_train.close()

if __name__ == '__main__':
    # 输入的文件夹
    dirname ='../../data/doneDocsDescription'
    #存储的txt文件,这里定好名字就行，不存在会自行创造，存在应该会重写
    save_file_name='../../data/tmp2/oneline.txt'
    # 存储txt文件的名字和序号对应的词典  {序号：名字}
    picklefile = '../../data/tmp/names.txt'
